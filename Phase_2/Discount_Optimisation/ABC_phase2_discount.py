...
def sigmoid(x):  # a sigmoid function used for customer no prediction
...
for discount_fraction in np.linspace(0.2, 0.4, 8):
    n_customers = int(49.5*(1 + sigmoid((discount_fraction*100 - 18) / 4)))         # no. of customers at a particular day

    # Initializing shop parameters and customers data
    class Barber_shop:
        def __init__(self, n_customers, n_barbers, operation_list, customer_type, schedule, discount_fraction, member_fees):   # for initialization for class barber shop
            ...
            self.membership = member_fees    # fee to become a premium member
        ...
        def objective(self, schedule, time_required):
            ...
            for j in range(self.n_customers):
                ...
                cj = self.price[j]
                net_profit += (0.5 - wj * pj) * cj + (wj - 1) * self.membership   # profit for each customer
            return net_profit
    ...
    def generate_customer_type(n_customers):
        std = int(n_customers - (0.3**(member_fees/200)) * n_customers)     # considering 70% are normal customers at 200 member fees
    ...
    # Calling Class
    shop_init = Barber_shop(n_customers, n_barbers, operation_list, customer_type, random_sol, discount_fraction, member_fees)  # initialise problem parameters
    ...
    loop_no.append(discount_fraction * 100)

# Plot Profit vs Iteration
plt.plot(loop_no, Objective)
plt.show()