...
class Genetic_Algorithm:
    def __init__(self, n_customers, n_barbers, operation_list, customer_type, max_obj_var, discount):
        self.barber_model = BS(n_customers, n_barbers, operation_list, customer_type, discount)
        self.objlist = []
        self.objolist = []
    ...
    def Initial_Parent_Generator(self, n_customers):
        ...
        for i in range(initial_pool):
            ...
            p1.extend(n_customers + 1 + i for i in range(n_customers - split1))
            p2.extend(
                2 * n_customers - split1 + 1 + i
                for i in range(n_customers - split2 + split1)
            )
            p3.extend(3*n_customers - split2 + 1 + i for i in range(split2))
    ...
    def roulette_wheel_selection(self, Generation, objective): #parent selection
        ...
        chromosome_probabilities = [
            objective[i] / population_fitness for i in range(0, 10)
        ]
